...
const ZIP_DEFAULT = '55378'; // Savage, MN
const ADDRESS_DEFAULT = 'Savage, MN';
// CRM_API_TOKEN is shared across every site in the portfolio and the CRM's
// PushLead schema has no site-id field, so this tag is the only way to trace
// a lead back to this domain. Must be updated if this codebase is ever
// copied to a different site.
const SITE_SOURCE_TAG = 'Site: commercialcleaningservicessavage.com';
...
function buildWizardPayload(body) {
  ...
  const visitorNotes = clean(input.notes);
  const notes = visitorNotes ? `${SITE_SOURCE_TAG} | ${visitorNotes}` : SITE_SOURCE_TAG;
